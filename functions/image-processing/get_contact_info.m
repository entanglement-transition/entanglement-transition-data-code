function contact_table = get_contact_info2(voxel_list,stack,thin_rad,thick_rad,min_d)
...
for i_rod = 1:num_tracked
    ...
    [rod_thick_i,ind_thick_i] = get_rod_from_centerline(rr_i,thick_rad,stack);
    rod_thin_i = get_rod_from_centerline(rr_i,thin_rad,stack);
    ...
    for j_rod = 1:num_nearby_rods
        ...
        [rod_thick_j,ind_thick_j] = get_rod_from_centerline(rr_j,thick_rad,stack);
        contact_indices = intersect(ind_thick_i,ind_thick_j);

        if ~isempty(contact_indices)
            num_contact_voxels(end+1) = numel(contact_indices);
            highlight = [rod_thick_i;rod_thick_j];
            ...
            num_voxels_combined = (numel(ind_thick_i) + numel(ind_thick_j)) + numel(contact_indices)*2;
            ...
            if 0
                ...
                tiledlayout(1,4)
                nexttile([1,3])
                plot3v(rod_thick_i,'k.');hold on;
                plot3v(rod_thick_j,'k.');axis equal;
