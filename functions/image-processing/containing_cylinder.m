function [sweep,indices] = containing_cylinder(stack_size,centerline,r)
    % get voxel indices from inequalities
%     assert(mod(r,2) == 1)
    centerline = unique(round(centerline),'rows','stable');    
    
    [cen,ori,slist] = get_line_coord(centerline);
    
    delta = max(slist) - min(slist);
    slist = linspace(min(slist)-delta*0.5,max(slist)+delta*0.5,round(delta*2));
    
    centerline = cen+ori.*slist';
