function [centerlines,tangents,found_points,matching_errors] = segmentation_by_local_sampling(stack,initial_point,initial_axis,rad,lwin,marching_step)
...
centerlines = flipud(centerlines(2:end,:));
tangents = -flipud(tangents(2:end,:));
...
while 1 % going "right"
    ...
    %         plot3v(centerline_point,'o');hold on;axis equal;
    k = k + 1;
    if k > 500
        ;
        break;
...
% remove centerpoint to prevent duplicate
[centerlines,ia,ic] = unique(centerlines,'rows','stable');
tangents = tangents(ia,:);
